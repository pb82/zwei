...
void Player::init() {
    ...
    // sound effects
    sound_tasks.emplace_back(SOUND_PICKUP, std::async(std::launch::deferred, load_sound, "assets/RAW/pickup2.wav"));

    // music
    // deferred load, async seems to cause random crashes
    music_tasks.emplace_back(MUSIC_1, std::async(load_music, "assets/RAW/sample.ogg"));
    music_tasks.emplace_back(MUSIC_GAMEOVER, std::async(std::launch::deferred, load_music, "assets/RAW/gameover.ogg"));
